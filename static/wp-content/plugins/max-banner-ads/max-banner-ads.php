...
class MBPBannerAdsPlugin extends MBPBannerAds
{
	...
	function mbanOptionsPg() {
		...
		if ( ((trim($_GET['submit']) != '' && trim($_GET['from']) != '') || trim($_GET['submit_again']) != '') && $this->mban_activate != 2 ) { 
			update_option('mban_name', $_GET['name']);
			update_option('mban_email', $_GET['from']);
		...
		} else if ( intval($this->mban_activate) == 2 ) { // Options page
...
function mba_display_zone($zone_id) {
	...
	if ( !is_admin() && $MBPBannerAdsPlugin->mban_activate == 2  ) {
		if ( intval($zone_id) > 0 ) {
...
function mbanWidgetInit() {
	...
	if ( !function_exists('register_sidebar_widget') || !function_exists('register_widget_control') || $MBPBannerAdsPlugin->mban_activate != 2 ) {
		return; 
	}
