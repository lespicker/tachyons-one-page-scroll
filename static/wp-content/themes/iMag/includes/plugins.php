...
function src_simple_recent_comments($src_count=8, $src_length=35, $pre_HTML='', $post_HTML='') {
	...
	foreach ($comments as $comment) {
		$output .= "\n\t<li><span>" . $comment->comment_author . ':</span> <a href="' . get_permalink($comment->ID) . '#comment-' . $comment->comment_ID  . '" title="on ' . $comment->post_title . '">' . strip_tags($comment->com_excerpt) . '...</a></li>';
	}
	$output .= $post_HTML;
