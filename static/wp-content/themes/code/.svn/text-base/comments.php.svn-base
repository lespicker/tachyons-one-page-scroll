...
<?php if ($comments) : ?>
	<div id="comments">
		...
		<?php foreach ($comments as $comment) : ?>
			<li id="comment-<?php comment_ID() ?>"  class="comment <?php if (function_exists('author_highlight')) { author_highlight(); }?>">
				<?php if ($comment->comment_approved == '0') : ?>
				<p class="moderation"><?php _e('Your comment will be published when it is approved.','vostok'); ?></p>
				<?php endif; ?>
				<cite><span class="date"><?php comment_date(__('j/m/Y','vostok')) ?></span><?php comment_author_link() ?> say:</cite>
				<blockquote>
					<?php comment_text() ?>
				</blockquote>
			</li>
	...
	<?php else : ?>
		...
		<?php else : ?>
			<p>
				...
				<label for="author"><?php _e('Name','vostok'); ?><?php if ($req) _e('(required)','vostok'); ?></label>
			</p>
			<p>
				<input type="text" name="email" id="email" class="input-text" value="<?php echo $comment_author_email; ?>" size="22" />
				<label for="email"><?php _e('Email','vostok'); ?> <?php if ($req) _e('(required, but it will not be published)','vostok'); ?></label>
			</p>
			<p>
				<input type="text" name="url" id="url" class="input-text" value="<?php echo $comment_author_url; ?>" size="22" />
				<label for="url"><?php _e('Website (if you have)','vostok'); ?></label>
			</p>
		<?php endif; ?>
			<p>
